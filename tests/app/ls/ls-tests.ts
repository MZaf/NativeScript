...
import { livesync, getCssFileName, setCssFileName } from "tns-core-modules/application/application-common";
...
export function test_livesync1() {
    console.log("--> start");
    let page0;
    let page1;
    ...
    const start = helper.getCurrentPage();
    // livesync({ type: HmrType.style, module: "./app.css" });
    livesync({ type: "style", module: "./app.css" });
    const end = helper.getCurrentPage();

    TKUnit.assertTrue(end._cssState.isSelectorsLatestVersionApplied());
    TKUnit.assertEqual(start, end, "Fail!");

    setCssFileName("./lqlq.css");
    console.log(getCssFileName());
